"""Read-only live preview of Jump Trading and XTX Markets, without database writes or notifications."""
...
async def main():
    ...
    try:
        for source in ["jump_trading", "xtx_markets"]:
            collection = await build_collector(source, config.companies[source], http).collect()
            path = Path(f"data/discovery/lot12/{source}-preview.json")
