"""Read-only previews of Nomura professional board, without database writes or alerts."""
...
async def main():
    ...
    try:
        for source in ["nomura_professionals"]:
            collection = await build_collector(source, config.companies[source], http).collect()
            path = Path(f"data/discovery/lot14/{source}-preview.json")
