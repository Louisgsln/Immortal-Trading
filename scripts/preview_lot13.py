"""Read-only previews of UBS and HSBC professional boards, without database writes or alerts."""
...
async def main():
    ...
    try:
        for source in ["ubs_professionals", "hsbc_professionals"]:
            collection = await build_collector(source, config.companies[source], http).collect()
            path = Path(f"data/discovery/lot13/{source}-preview.json")
