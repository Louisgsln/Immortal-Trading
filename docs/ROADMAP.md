...
4. **Poursuivre l'audit des sources** : continuer les actualisations bornées avec aperçu d’impact sur les quinze sources restantes. Le lot 42 a rafraîchi IMC, DRW, Flow, Jump et XTX ; le lot 43 ajoute les quatre banques Workday. Les cinq absentes Greenhouse et 29 absentes Workday n’ont pas été clôturées ni artificiellement rafraîchies. Examiner séparément les nouveaux cas DRW `Quant Researcher` et `Junior Trader` présent dans la description. Les lots 52, 57, 58, 61 et 63 affichent les mentions de diplôme DRW, IMC, Jump Trading, Goldman professionnels, Citi et le champ Qualification de Nomura professionnels (97 offres couvertes) ; étendre la couverture après audit des rubriques des autres employeurs. Le lot 30 a terminé quatre recherches Citi filtrées sans nouveau chemin retenu ; les autres catégories et la recherche `structuring` restent hors de cette mesure. Reprendre JPMorgan lorsque son accès public devient disponible. Aucune exhaustivité globale n'est revendiquée.
...
Dashboard en lecture seule livré au lot 22, statistiques historiques au lot 24, suivi interactif local au lot 25 ; rappels conditionnels livrés au lot 17. Les lots 55, 58, 61 et 63 livrent une première lecture synthétique des missions par extraits employeur vérifiés (DRW, IMC, HSBC professionnels, Jump Trading, Goldman Sachs professionnels, Citi), couvrant 117 offres sur la copie auditée. Restent l’extension à d’autres rubriques auditées, l'enrichissement IA facultatif et le matching CV. Les fonctions de candidature automatique, les recherches de contacts privés et les prédictions de calendrier ne font pas partie de ce premier lot.
